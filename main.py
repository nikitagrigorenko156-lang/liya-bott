...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
✅ НОВОЕ: Сохранение данных в JSON (выживает рестарт)
✅ НОВОЕ: Счётчик сообщений + дневной лимит
✅ НОВОЕ: Реферальная система (+бонусные дни)
✅ НОВОЕ: Напоминания (/remind)
✅ НОВОЕ: Уведомления админу о новых юзерах
✅ НОВОЕ: Полная аналитика (DAU, топ юзеры, рост)
✅ НОВОЕ: Автоматическая оплата Telegram Stars
✅ НОВОЕ: Пробный период (3 дня бесплатно)
✅ НОВОЕ: Дневник/заметки голосом
✅ НОВОЕ: Экспорт данных для админа
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
FREE_MSG_LIMIT  = 10           # бесплатных сообщений без пробника (для тех кто не активировал триал)
DAILY_MSG_LIMIT = 999          # лимит сообщений в день для платных (фактически безлимит)
FREE_DAILY_LIMIT= 20           # лимит в день на пробном периоде
DATA_FILE       = "liya_data.json"   # файл хранения данных
LOG_FILE        = "liya_log.txt"     # лог событий
...
# ══════════════════════════════════════════════
#  ХРАНИЛИЩЕ ДАННЫХ (персистентное)
# ══════════════════════════════════════════════

class DataStore:
    """Сохраняет всё в JSON-файл. Переживает рестарты Render."""

    def __init__(self, filepath=DATA_FILE):
        self.filepath = filepath
        self._lock = threading.Lock()
        self.data = {
            "paid_users":    {},   # {uid: {"expires": ISO или null, "plan": "forever/trial/paid"}}
            "blocked":       [],   # [uid, ...]
            "referrals":     {},   # {uid: {"code": str, "invited": [uid,...], "bonus_days": int}}
            "stats":         {},   # {uid: {"total_msgs": int, "daily": {"2024-01-01": int}, "joined": ISO, "username": str, "name": str}}
            "reminders":     {},   # {uid: [{"text": str, "time": "HH:MM", "daily": bool}, ...]}
            "notes":         {},   # {uid: [{"text": str, "date": ISO}, ...]}
            "msg_counts":    {},   # {uid: {"date": "2024-01-01", "count": int}}
        }
        self.load()
    ...
    def load(self):
        try:
            ...
            log_event(f"DataStore load error: {e}")
    ...
    def save(self):
        with self._lock:
            try:
                ...
                log_event(f"DataStore save error: {e}")

    # ── Пользователи ──
    def get_user(self, uid):
        return self.data["paid_users"].get(str(uid))

    def set_user(self, uid, expires, plan="paid"):
        self.data["paid_users"][str(uid)] = {
    ...
    def remove_user(self, uid):
        self.data["paid_users"].pop(str(uid), None)
        self.save()
    ...
    def has_access(self, uid, username=""):
        ...
        if uid in self.data["blocked"]:
            return False
        u = self.data["paid_users"].get(uid)
        if not u:
            return False
    ...
    def is_blocked(self, uid):
        return str(uid) in self.data["blocked"]

    # ── Счётчик сообщений ──
    def count_message(self, uid):
        """Увеличивает счётчик. Возвращает (today_count, total_count)."""
        uid   = str(uid)
        today = datetime.now().strftime("%Y-%m-%d")

        # Дневной счётчик
        mc = self.data["msg_counts"].get(uid, {"date": today, "count": 0})
        if mc["date"] != today:
            mc = {"date": today, "count": 0}
        mc["count"] += 1
        self.data["msg_counts"][uid] = mc

        # Общая статистика
        st = self.data["stats"].get(uid, {})
        st["total_msgs"] = st.get("total_msgs", 0) + 1
        daily = st.get("daily", {})
        daily[today] = daily.get(today, 0) + 1
        st["daily"] = daily
        self.data["stats"][uid] = st
    ...
    def register_user(self, uid, username, name):
        ...
        if uid not in self.data["stats"]:
            ...
            self.data["stats"][uid]["username"] = username
            self.data["stats"][uid]["name"]     = name
            self.save()
            return False

    def get_analytics(self):
        total_users = len(self.data["stats"])
        paid_users  = len(self.data["paid_users"])
        blocked     = len(self.data["blocked"])
        today       = datetime.now().strftime("%Y-%m-%d")
        yesterday   = (datetime.now() - timedelta(days=1)).strftime("%Y-%m-%d")
        dau_today   = sum(1 for s in self.data["stats"].values()
                         if today in s.get("daily", {}))
        dau_yest    = sum(1 for s in self.data["stats"].values()
                         if yesterday in s.get("daily", {}))
        total_msgs  = sum(s.get("total_msgs", 0) for s in self.data["stats"].values())
        # Топ юзеры
        top = sorted(self.data["stats"].items(),
                    key=lambda x: x[1].get("total_msgs", 0), reverse=True)[:5]
        # Новые за 7 дней
        week_ago = (datetime.now() - timedelta(days=7)).isoformat()
        new_week = sum(1 for s in self.data["stats"].values()
                      if s.get("joined", "0") > week_ago)
        return {
    ...
    def get_ref_code(self, uid):
        ...
        if uid not in self.data["referrals"]:
            code = f"ref{uid}"
            self.data["referrals"][uid] = {"code": code, "invited": [], "bonus_days": 0}
            self.save()
        return self.data["referrals"][uid]["code"]

    def apply_referral(self, new_uid, ref_code):
        """Применяет реферальный код. Возвращает uid пригласившего или None."""
        new_uid = str(new_uid)
        for owner_uid, ref_data in self.data["referrals"].items():
            if ref_data["code"] == ref_code and new_uid not in ref_data["invited"] and owner_uid != new_uid:
                ref_data["invited"].append(new_uid)
                ref_data["bonus_days"] = ref_data.get("bonus_days", 0) + 7
                # Добавляем 7 дней к подписке пригласившего
                ...
                elif current and current.get("expires") is None:
                    pass  # бессрочная, не меняем
                else:
                    new_exp = datetime.now() + timedelta(days=7)
    ...
    def add_note(self, uid, text):
        uid = str(uid)
        if uid not in self.data["notes"]:
            self.data["notes"][uid] = []
        self.data["notes"][uid].append({"text": text, "date": datetime.now().isoformat()})
        # Храним последние 50
        if len(self.data["notes"][uid]) > 50:
            self.data["notes"][uid] = self.data["notes"][uid][-50:]
        self.save()

    def get_notes(self, uid):
        return self.data["notes"].get(str(uid), [])

    # ── Напоминания ──
    def add_reminder(self, uid, text, time_str, daily=False):
        uid = str(uid)
        if uid not in self.data["reminders"]:
            self.data["reminders"][uid] = []
        self.data["reminders"][uid].append({
    ...
    def get_reminders(self, uid):
        return self.data["reminders"].get(str(uid), [])

    def remove_reminder(self, uid, idx):
        uid = str(uid)
        reminders = self.data["reminders"].get(uid, [])
...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        a = db.get_analytics()
        resp = (f"Liya Bot v2.0 | Users: {a['total_users']} | "
...
def reminder_scheduler():
    """Каждую минуту проверяет напоминания и отправляет их."""
    while True:
        try:
...
def check_daily_limit(uid, username=""):
    """Проверяет дневной лимит. Возвращает (ok, count, limit)."""
    if is_vip(username): return True, 0, 999
    user = db.get_user(uid)
    if not user:
        # Нет подписки — ограниченный доступ
    ...
    return True, count, DAILY_MSG_LIMIT  # платная — безлимит
...
def notify_admin(text):
    ...
    for admin_name in ADMIN_USERNAMES:
        # Ищем uid админа в stats
        for uid_str, stat in db.data["stats"].items():
            if stat.get("username", "").lower() == admin_name:
                try:
...
def check_and_count(msg):
    """Проверяет доступ + дневной лимит + считает сообщение."""
    uid      = msg.from_user.id
    username = msg.from_user.username or ""
...
def _do_summarize(chat_id, uid, text, reply_to_msg=None):
    ...
    try:
        ...
        target = reply_to_msg or None
        if target:
            bot.reply_to(target, answer, reply_markup=kb)
        else:
...
def cmd_ref(msg):
    ...
    ref_data = db.data["referrals"].get(str(uid), {})
...
def handle_voice(msg):
    ...
    try:
        ...
        # Проверяем — это заметка или вопрос?
...
def handle_callback(call):
    ...
    # Эти кнопки доступны без подписки
    free_callbacks = {"btn_menu", "btn_help", "btn_trial", "btn_pay_stars",
    ...
    if data == "btn_trial":
        ...
        notify_admin(f"🎁 Пробный период: {uid} @{call.from_user.username or ''}")
        return
    ...
    if data == "btn_pay_stars":
        ...
        kb = types.InlineKeyboardMarkup(row_width=1)
        kb.add(
            types.InlineKeyboardButton(f"⭐ {PRICE_STARS} Stars — 30 дней",
                callback_data="pay_stars_30"),
            types.InlineKeyboardButton("⭐ 200 Stars — 90 дней",
                callback_data="pay_stars_90"),
            types.InlineKeyboardButton("⭐ 500 Stars — Навсегда",
                callback_data="pay_stars_forever"),
    ...
    if data.startswith("pay_stars_"):
        ...
        try:
            bot.send_invoice(
                ...
                provider_token="",  # Stars не нужен provider_token
                currency="XTR",
    ...
    if data == "btn_account":
        ...
        stat      = db.data["stats"].get(str(uid), {})
    ...
    if data == "btn_notes":
        ...
        if not notes:
            ...
            for n in reversed(last_5):
                date = n["date"][:10]
                lines.append(f"📅 {date}\n{n['text'][:200]}\n")
            bot.send_message(uid, f"📓 Последние заметки:\n\n" + "\n".join(lines), reply_markup=kb)
        return
    ...
    # ── АДМИН-ПАНЕЛЬ ──
    if data == "adm_panel":
        username_cb = call.from_user.username or ""
        if not is_admin(username_cb):
    ...
    if data.startswith("adm_") and not is_admin(call.from_user.username or ""):
        bot.answer_callback_query(call.id, "❌ Нет доступа!"); return
    ...
    if data == "adm_stats":
        ...
        a    = db.get_analytics()
        top  = "\n".join(
            f"  {i+1}. ID{uid_str}: {stat.get('total_msgs',0)} сообщений (@{stat.get('username','')})"
            for i, (uid_str, stat) in enumerate(a["top_users"])
        )
        bot.send_message(uid,
            f"📊 Полная аналитика\n\n"
            f"👥 Всего пользователей: {a['total_users']}\n"
        ...
        bot.answer_callback_query(call.id)
        all_users = list(db.data["paid_users"].keys())
        if not all_users:
        ...
        for u in all_users[:15]:
            ...
            stat     = db.data["stats"].get(u, {})
            uname    = stat.get("username", "")
            uname_str = f"@{uname}" if uname else ""
            kb.add(types.InlineKeyboardButton(
                f"👤 {u} {uname_str} | {status}",
                callback_data=f"adm_user_{u}"))
        kb.add(types.InlineKeyboardButton("🔙 Назад", callback_data="adm_back"))
        bot.send_message(uid, f"👥 Пользователи ({len(all_users)}):", reply_markup=kb)

    elif data.startswith("adm_user_"):
        target = int(data.replace("adm_user_", ""))
        bot.answer_callback_query(call.id)
        status   = db.sub_status(target)
        stat     = db.data["stats"].get(str(target), {})
        uname    = stat.get("username", "")
        total    = stat.get("total_msgs", 0)
        joined   = stat.get("joined", "")[:10]
        kb = types.InlineKeyboardMarkup(row_width=2)
        kb.add(
        ...
        # формат: adm_time_{target_uid}_{days}
        parts    = data.split("_")
        target   = int(parts[2])
        days     = int(parts[3])
    ...
    # ── Режимы ──
        ...
        bot.send_chat_action(uid, "typing")
        try:
            answer = ask_ai(uid, "Дай мощную мотивационную речь для девушки которой нужна поддержка. Искренне и с душой.",
        ...
        bot.send_chat_action(uid, "typing")
        try:
            prompt = (f"Проведи тест: '{test_name}'. "
                     f"Задай 5 интересных вопросов по одному. Начни с первого вопроса прямо сейчас.")
        ...
        bot.send_message(uid, f"✅ Правильный ответ:\n\n{answer}\n\n🌟 Факт:\n{fact}", reply_markup=kb)
        modes[uid] = "normal"
        ...
        try:
            prompt = (f"Вот текст:\n{text}\n\n"
                     f"Объясни это максимально просто — как подруга подруге. "
...
def handle_text(msg):
    ...
    if current_mode == "note_add_mode":
        ...
        bot.reply_to(msg, f"📓 Заметка сохранена!", reply_markup=kb)
        return
    ...
    if current_mode == "adm_broadcast_mode":
        modes[uid] = "normal"
        if not is_admin(msg.from_user.username or ""): return
        all_ids = set(db.data["paid_users"].keys())
        sent, failed = 0, 0
    ...
    if current_mode == "todo_add_mode":
        ...
        bot.reply_to(msg, f"✅ Добавила: {text}", reply_markup=kb)
        return
...
print(f"👑 Бот '{BOT_NAME}' ULTRA LEGENDARY v2.0 запущен!")
